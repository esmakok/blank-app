...
def show_visualization():
    ...
    el31_ready = 'zip_buffer_el31' in st.session_state
    zblir_ready = 'zip_buffer_zblir' in st.session_state
    zdm240_ready = 'df_zdm240_cleaned' in st.session_state

    all_names = set()

    if el31_ready:
        el31_zip = zipfile.ZipFile(st.session_state.zip_buffer_el31)
        all_names.update([f.replace(".csv", "").replace("-A", "").replace("-AB", "") for f in el31_zip.namelist()])
    if zblir_ready:
        zblir_zip = zipfile.ZipFile(st.session_state.zip_buffer_zblir)
        all_names.update([f.replace(".csv", "").replace("-A", "").replace("-AB", "") for f in zblir_zip.namelist()])
    if zdm240_ready:
        zdm_names = [str(t) for t in st.session_state.df_zdm240_cleaned["Tesisat"].unique()]
        all_names.update(zdm_names)

    if not all_names:
        st.warning("Hiçbir geçerli tesisat verisi bulunamadı.")
        return

    selected = st.selectbox("Bir tesisat seçin:", sorted(all_names))

    # EL31
    if el31_ready:
        file = next((f for f in el31_zip.namelist() if f.startswith(selected)), None)
        if file:
            df = pd.read_csv(el31_zip.open(file), sep=";")
            st.subheader("P Endeksi")
            st.pyplot(plot_el31_graph(df))
        else:
            st.info("Bu tesisat için EL31 verisi yok.")
    else:
        st.info("P Endeksi için EL31 dosyası yüklenmeli.")

    # ZBLIR
    if zblir_ready:
        file = next((f for f in zblir_zip.namelist() if f.startswith(selected)), None)
        if file:
            df = pd.read_csv(zblir_zip.open(file), sep=";")
            for endeks in ["T1", "T2", "T3"]:
                st.subheader(f"{endeks} Endeksi")
                st.pyplot(plot_zblir_graph(df, endeks))
        else:
            st.info("Bu tesisat için ZBLIR verisi yok.")
    else:
        st.info("T1–T3 grafiklerini görmek için ZBLIR dosyası yüklenmeli.")

    # ZDM240
    if zdm240_ready:
        df_zdm = st.session_state.df_zdm240_cleaned
        selected_int = int(selected) if selected.isdigit() else selected
        df_filtered = df_zdm[df_zdm["Tesisat"] == selected_int]
        if not df_filtered.empty:
            st.subheader("ZDM240 Tüketim Grafiği")
            st.pyplot(plot_zdm240_graph(df_filtered))
        else:
            st.info("Bu tesisat için ZDM240 verisi yok.")
    else:
        st.info("ZDM240 grafiği için ilgili dosya yüklenmeli.")

# ===============================
# 📌 GÖRSELLEŞTİRME BUTONU
# ===============================
